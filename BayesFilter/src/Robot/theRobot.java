...
public class theRobot extends JFrame {
    ...
    boolean isWall(int row, int col) {
        return mundo.grid[col][row] == 1;
    }

    String findActual(int row, int col) {
        String retVal = "";
        retVal += ((isWall(row-1, col)) ? "1" : "0");
        retVal += ((isWall(row+1, col)) ? "1" : "0");
        retVal += ((isWall(row, col+1)) ? "1" : "0");
        retVal += ((isWall(row, col-1)) ? "1" : "0");
        return retVal;
    }

    void transitionUpdate(int action) {
        double[][] newProbs = new double[probs.length][probs[0].length];
        for (int row = 1; row < probs.length - 1; ++row) {
            for (int col = 1; col < probs[row].length - 1; ++col) {
                if (isWall(row, col)) {
                    continue;
                }

                newProbs[col][row] += ((action==0) ? probs[col][row+1]*moveProb : probs[col][row+1]*((1-moveProb)/4));
                newProbs[col][row] += ((action==1) ? probs[col][row-1]*moveProb : probs[col][row-1]*((1-moveProb)/4));
                newProbs[col][row] += ((action==2) ? probs[col-1][row]*moveProb : probs[col-1][row]*((1-moveProb)/4));
                newProbs[col][row] += ((action==3) ? probs[col+1][row]*moveProb : probs[col+1][row]*((1-moveProb)/4));
                newProbs[col][row] += ((action==4) ? probs[col][row]*moveProb : probs[col][row]*((1-moveProb)/4));
            }
        }
        probs = newProbs;
    ...
    void sensorUpdate(String sonars) {
        for (int row = 1; row < probs.length - 1; ++row) {
            for (int col = 1; col < probs[row].length - 1; ++col) {
                if (isWall(row, col)) {
                    continue;
                }
                String perfect = findActual(row, col);
                int m = 0;
                for (int i = 0; i < perfect.length(); ++i) {
                    if (perfect.charAt(i) == sonars.charAt(i)) {
                        ++m;
                    }
                }

                probs[col][row] *= Math.pow(sensorAccuracy, m) * Math.pow(1.0-sensorAccuracy, 4-m);
            }
        }
    }

    void normalize() {
        double sum = Arrays.stream(probs).flatMapToDouble(Arrays::stream).sum();
        for (int row = 0; row < probs.length - 1; ++row) {
            for (int col = 0; col < probs[row].length - 1; ++col) {
                probs[col][row] /= sum;
            }
        }
    }
