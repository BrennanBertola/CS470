...
def tiles_out_of_row_column(puzzle):
    ...
    retVal = 0
    for i in range(1, len(puzzle.state)):
        currIdx = puzzle.state.index(i)

        #checks if in correct row
        if currIdx // 3 != i // 3:
            retVal += 1
        #checks if in correct col
        if currIdx % 3 != i % 3:
            retVal += 1
...
def manhattan_distance_to_goal(puzzle):
    ...
    for i in range(1, len(puzzle.state)):
        ...
        #distance of the rows
        retVal += abs(currIdx // 3 - i // 3)
        #distance of the cols
        retVal += abs(currIdx % 3 - i % 3)
...
def run_iterative_search(start_node):
    ...
    depth_limit = start_node.f_value
    ...
    while depth_limit < max_depth_limit:
        ...
        path_length, newDepth = run_dfs(start_node, depth_limit, visited)
...
def run_dfs(node, depth_limit, visited):
    ...
    #track the new min depth
    minDepth = node.f_value if node.f_value > depth_limit else math.inf
    ...
    for m in moves:
        ...
        if node.puzzle.id() not in visited:
            ...
            path_length, foundDepth = run_dfs(node, depth_limit, visited)
            ...
            #update the minDepth
            if foundDepth < minDepth and foundDepth > depth_limit:
                minDepth = foundDepth
