...
class Sudoku():
    ...
    def forward_check(self, row, column, value, mode='remove'):
        ...
        # self.cells[row][column].assign_value(value)
        count = 0
        ...
        for r in range(9):
            ...
            #if current cell has value increment count
            if self.cells[r][column].has_value(value):
                count += 1
            #if mode is remove, remove value and return false if empty
            if mode == 'remove':
        ...
        for c in range(9):
            if c == column:
                continue
            if self.cells[row][c].value is not None:
                continue

            if self.cells[row][c].has_value(value):
                count += 1
            if mode == 'remove':
                if not self.cells[row][c].remove_value(value):
                    return False

        currGrid, currCell = self.get_grid_cell(row, column)
        for cell in range(9):
            currRow, currCol = self.get_row_column(currGrid, cell)
            if cell == currCell:
                continue
            if self.cells[currRow][currCol].value is not None:
                continue

            if self.cells[currRow][currCol].has_value(value):
                count += 1
            if mode == 'remove':
                if not self.cells[currRow][currCol].remove_value(value):
                    return False
...
def mrv(puzzle:Sudoku, unassigned):
    '''
    ...
    #goes through entire list with a running min
    for curr in unassigned:
        currVal = len(puzzle.cells[curr[0]][curr[1]].domain)
        if currVal < minVal:
            retVal = [curr]
            minVal = currVal
        elif currVal == minVal:
...
def count_constraints(puzzle: Sudoku, row, column):
    '''
    ...
    # TASK 3 CODE HERE
    count = 0
    grid, _ = puzzle.get_grid_cell(row, column)
    for i in range(9):
        tmpR, tmpC = puzzle.get_row_column(grid, i)
        #count values in same column
        if puzzle.cells[i][column].value is None:
            count += len(puzzle.cells[i][column].domain)
        #count values in same row, but not the original cell
        if puzzle.cells[row][i].value is None and i != column:
            count += len(puzzle.cells[row][i].domain)
        #count values in same grid, but not same row/column
        if puzzle.cells[tmpR][tmpC].value is None and tmpR != row and tmpC != column:
            count += len(puzzle.cells[tmpR][tmpC].domain)
...
def select_variable(puzzle, use=True):
    '''
    ...
    #if not being used, return random unassigned variable
    if not use:
        tmp = random.choice(unassigned)
        return tmp[0], tmp[1]
...
def order_values(puzzle:Sudoku, row, column, use=True):
    '''
    ...
    #Used for write up testing, just returns the domain unsorted
    if not use:
        return domain

    #Finds the count for all values in a domain
    for value in domain:
        count = puzzle.forward_check(row, column, value, 'count')
        #if the count is in the map, add the domain value to it's list
        if count in counts:
            counts[count].append(value)
        #else add the count to the map and the ordered list
        else:
            counts[count] = [value]
            orderedCounts.append(count)

    #sort the list
    orderedCounts = sorted(orderedCounts)
    retVal = []
    #add the values into the result list
    for count in orderedCounts:
        retVal += counts[count]

    return retVal

def backtracking_search(puzzle: Sudoku):
    '''
    ...
    for value in values:
        ...
        copy.cells[row][col].assign_value(value)
        ...
        success = copy.forward_check(row, col, value)
        ...
        recurseResults = backtracking_search(copy)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        for ps in pf.readlines():
            ...
            # Stop when we solve enough (changed to >=)
